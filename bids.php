...
<?php
  /*
   / A similar page to mybids.php to support user to view others' bids.
   / May want to use this mote general page to replace the default mybids.php instead.
  */
  session_start();
  if(!is_login()) redirect($_SERVER['HTTP_REFERER'] or 'index.php', 'You are not logged in.'); // must login to use it for simplicity.

  // IMPROVE: More properly deal with user not exist and uses username
  $user_id = isset($_GET['user']) ? $_GET['user'] : $_SESSION['user'];
  echo sprintf('<h2 class="my-3">Bids from %s</h2>', $user_id);

  try{
    $results = get_bids_from_user($user_id);
  } catch(Exception $e){
    error_log($e);
